                                                                    ...
                                                                    <div class="bed_content">
                                                                        ...
                                                                        <?php if (isset($sroom['bed']) && $sroom['bed']) { ?>
                                                                            <?php foreach ($sroom['bed'] as $bed) { ?>
                                                                                <div data-stat="<?= $bed['stat'] == Bed::STAT_OPEN ? (isset($total['bed_check'][$bed['id']]) ? ( $total['bed_check'][$bed['id']] == CheckOrder::STAT_CHECKIN ? 'bg-blue' : 'bg-green') : 'bg-gray') : 'bg-red' ?>" data-gender="<?= $bed['stat'] == Bed::STAT_OPEN ? (isset($total['bed_student'][$bed['id']]['gender']) ? ($total['bed_student'][$bed['id']]['gender'] == Student::GENDER_MALE ? 'bg-aqua' : ($total['bed_student'][$bed['id']]['gender'] == Student::GENDER_FEMALE ? 'bg-fuchsia' : 'bg-yellow')) : (isset($total['bed_check'][$bed['id']]) ? 'bg-black' : 'bg-gray')) : 'bg-red' ?>" style="<?= count($sroom['bed']) > 5 ? 'height:50%;width:' . (100 / ceil(count($sroom['bed']) / 2)) . '%' : 'height:100%;width:' . (100 / count($sroom['bed'])) . '%' ?>" title="<?= isset($total['bed_student'][$bed['id']]['name']) ? $total['bed_student'][$bed['id']]['name'] : $bed['note'] ?>" data-toggle="tooltip"><?= $bed['name'] ?></div>
                                                                            <?php } ?>
                                                                        <?php } else { ?>
                                                    ...
                                                    <div class="bed_content">
                                                        ...
                                                        <?php if (isset($broom['bed']) && $broom['bed']) { ?>
                                                            <?php foreach ($broom['bed'] as $bed) { ?>
                                                                <div data-stat="<?= $bed['stat'] == Bed::STAT_OPEN ? (isset($total['bed_check'][$bed['id']]) ? ( $total['bed_check'][$bed['id']] == CheckOrder::STAT_CHECKIN ? 'bg-blue' : 'bg-green') : 'bg-gray') : 'bg-red' ?>" data-gender="<?= $bed['stat'] == Bed::STAT_OPEN ? (isset($total['bed_student'][$bed['id']]['gender']) ? ($total['bed_student'][$bed['id']]['gender'] == Student::GENDER_MALE ? 'bg-aqua' : ($total['bed_student'][$bed['id']]['gender'] == Student::GENDER_FEMALE ? 'bg-fuchsia' : 'bg-yellow')) : (isset($total['bed_check'][$bed['id']]) ? 'bg-black' : 'bg-gray')) : 'bg-red' ?>" style="<?= count($broom['bed']) > 5 ? 'height:50%;width:' . (100 / ceil(count($broom['bed']) / 2)) . '%' : 'height:100%;width:' . (100 / count($broom['bed'])) . '%' ?>" title="<?= isset($total['bed_student'][$bed['id']]['name']) ? $total['bed_student'][$bed['id']]['name'] : $bed['note'] ?>" data-toggle="tooltip"><?= $bed['name'] ?></div>
                                                            <?php } ?>
                                                        <?php } else { ?>
                                                            <div style="border:none;float:none;line-height: 30px;font-size: 14px;">无</div>
                                                        <?php } ?>
