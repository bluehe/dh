...
//        $this->execute("set GLOBAL event_scheduler = ON;");
        ...
        //投诉表定时任务
        $suggest_table = '{{%suggest_order}}';
        $stat_e = \dms\models\Suggest::STAT_EVALUATE;
        $stat_r = \dms\models\Suggest::STAT_REPLY;
        $evaluate = \dms\models\Suggest::EVALUATE_VSAT;
        $evaluate_user = \dms\models\Suggest::EVALUATE_SYSTEM;
//        $this->execute("set GLOBAL event_scheduler = ON;");
        ...
        $this->execute("CREATE EVENT `event_suggestevaluate` ON SCHEDULE EVERY 1 DAY STARTS '2017-01-01 00:00:00'
            ...
            DO
            BEGIN
                UPDATE {$suggest_table} SET stat={$stat_e},evaluate={$evaluate_user},evaluate1={$evaluate},end_at=unix_timestamp(now()),note='系统默认' WHERE stat={$stat_r} AND reply_at<unix_timestamp(now())-3600*24*7;
                UPDATE {$table} SET exc_at=unix_timestamp(now()) WHERE name='event_suggestevaluate';
            END");
        $this->insert($table, ['id' => 2, 'name' => 'event_suggestevaluate', 'title' => '投诉回复7天后自动评价', 'start_at' => 1483200000, 'end_at' => NULL, 'interval_time' => 86400, 'content' => " UPDATE {$suggest_table} SET stat={$stat_e},evaluate={$evaluate_user},evaluate1={$evaluate},end_at=unix_timestamp(now()),note='系统默认' WHERE stat={$stat_r} AND reply_at<unix_timestamp(now())-3600*24*7", 'exc_at' => NULL, 'stat' => 1]);
    }
