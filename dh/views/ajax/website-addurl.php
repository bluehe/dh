...
<?php $this->beginBlock('submit') ?>
    $('body').off('submit').on('submit', '#website-form', function () {
        $.ajax({
            url: '<?= Url::toRoute(['ajax/website-addurl']) ?>',
            type: 'POST',
            data: $(this).serialize(),
            dataType: "json",
            success: function (data) {
                if (data.stat === 'success') {
                    ...
                    $('.category[id=' + data.cid + ']').find('.list-group').append(str);
                    if ($('.category[id=' + data.cid + ']').find('.website-content .list-group-item').length >= 10) {
                        $('.category[id=' + data.cid + ']').find('.add_page').hide();
                        $('.category[id=' + data.cid + ']').find('.websiteSortable').removeClass('sort');
                    }
